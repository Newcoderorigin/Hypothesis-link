...
class AppConfig:
    ...
    def from_env(cls, paths: AppPaths) -> "AppConfig":
        ...
        def _read_optional_int(
            env_name: str, key: str, default: Optional[int]
        ) -> Optional[int]:
            env_value = os.getenv(env_name)
            if env_value is not None:
                ...
                if parsed < 0:
                    raise ConfigurationError(
                        f"Environment variable {env_name} cannot be negative"
                    )
                return parsed
            ...
            try:
                ...
                raise ConfigurationError(
                    f"Configuration field {key} must be an integer"
                ) from exc
            if parsed < 0:
                raise ConfigurationError(f"Configuration field {key} cannot be negative")
            return parsed
        ...
        def _read_optional_str(env_name: str, key: str) -> Optional[str]:
            env_value = os.getenv(env_name)
            if env_value is not None:
        ...
        data = {
            ...
            "temperature": _read_float("HYPOTHESIS_TEMPERATURE", "temperature", 0.7),
            "max_tokens": _read_optional_int("HYPOTHESIS_MAX_TOKENS", "max_tokens", None),
            "api_key": _read_optional_str("HYPOTHESIS_API_KEY", "api_key"),
        }
...
class SecureHTTPClient:
    ...
    def __init__(
        ...
        self._session.headers.setdefault("User-Agent", "Hypothesis-link/2 (secure client)")
        if config.api_key:
            token = config.api_key.strip()
            if token:
                self._session.headers.setdefault("Authorization", f"Bearer {token}")
        self._lock = threading.Lock()
    ...
    def chat_completion(
        ...
        try:
            ...
            raise ConfigurationError("Temperature must be a numeric value") from exc
        ...
        try:
            ...
            raise ConfigurationError("max_tokens must be an integer value") from exc
        ...
        while True:
            ...
            try:
                ...
                detail = self._summarise_error_response(exc.response)
                raise APIError(f"Language model returned HTTP {status}: {detail}") from exc
            except ValueError as exc:
                self._metrics.record(time.perf_counter() - start, success=False)
    ...
    def list_models(self) -> List[str]:
        ...
        try:
            ...
            status = exc.response.status_code if isinstance(exc.response, Response) else None
            detail = self._summarise_error_response(exc.response)
            raise APIError(f"Model listing returned HTTP {status}: {detail}") from exc
        except ValueError as exc:
            self._metrics.record(time.perf_counter() - start, success=False)
    ...
    @staticmethod
    def _summarise_error_response(response: Optional[Response]) -> str:
        if response is None:
        ...
        try:
            ...
            detail = text[:300] or "no error details returned"
        else:
            detail = SecureHTTPClient._pluck_error_field(payload)
            if not detail:
                detail = json.dumps(payload)[:300]

        status_text = getattr(response, "reason", "").strip()
        status_prefix = f"{status_text}: " if status_text else ""
        return f"{status_prefix}{detail}" if detail else "no error details returned"

    @staticmethod
    def _pluck_error_field(payload: Any) -> Optional[str]:
        to_visit: List[Any] = [payload]
        seen: set[int] = set()
        interesting_keys = ("error", "message", "detail", "description", "info")
        ...
        while to_visit:
            ...
            if isinstance(current, bytes):
                try:
                    decoded = current.decode("utf-8", errors="replace").strip()
